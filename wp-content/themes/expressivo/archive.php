<?php
/**
 * The template for displaying Archive pages.
 *
 * Learn more: http://codex.wordpress.org/Template_Hierarchy
 *
 * @package expressivo
 */

// Get some options
$page_id = get_option('page_for_posts');
$blog_layout = get_field('blog_page_layout', $page_id);
$blog_layout_columns = get_field('blog_layout_columns', $page_id);

get_header(); ?>
<div id="content" class="site-content <?php if ($blog_layout == 'masonry') echo 'blog-masonry'; ?>" <?php if ($blog_layout == 'masonry') echo 'data-columns="' . $blog_layout_columns . '"'; ?>>
	<section id="primary" class="content-area">
		<main id="main" class="site-main" role="main">

		<?php if ( have_posts() ) : ?>

			<header class="page-header">
				<h1 class="page-title">
					<?php
						if ( is_category() ) :
							single_cat_title();

						elseif ( is_tag() ) :
							single_tag_title();

						elseif ( is_author() ) :
							printf( __( 'Author: %s', 'expressivo' ), '<span class="vcard">' . get_the_author() . '</span>' );

						elseif ( is_day() ) :
							printf( __( 'Day: %s', 'expressivo' ), '<span>' . get_the_date() . '</span>' );

						elseif ( is_month() ) :
							printf( __( 'Month: %s', 'expressivo' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'expressivo' ) ) . '</span>' );

						elseif ( is_year() ) :
							printf( __( 'Year: %s', 'expressivo' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'expressivo' ) ) . '</span>' );

						elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
							_e( 'Asides', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
							_e( 'Galleries', 'expressivo');

						elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
							_e( 'Images', 'expressivo');

						elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
							_e( 'Videos', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
							_e( 'Quotes', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
							_e( 'Links', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
							_e( 'Statuses', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
							_e( 'Audios', 'expressivo' );

						elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
							_e( 'Chats', 'expressivo' );

						else :
							_e( 'Archives', 'expressivo' );

						endif;
					?>
				</h1>
				<?php
					// Show an optional term description.
					$term_description = term_description();
					if ( ! empty( $term_description ) ) :
						printf( '<div class="taxonomy-description">%s</div>', $term_description );
					endif;
				?>
			</header><!-- .page-header -->

			<?php if ($blog_layout == 'masonry') echo '<ul class="blog-grid">'; ?>

			<?php while ( have_posts() ) : the_post(); ?>

				<?php if ($blog_layout == 'masonry') echo '<li class="blog-item">'; ?>

				<?php
					/* Include the Post-Format-specific template for the content.
					 * If you want to override this in a child theme, then include a file
					 * called content-___.php (where ___ is the Post Format name) and that will be used instead.
					 */
					get_template_part( 'templates/post-formats/blog', get_post_format() ); ?>

				<?php if ($blog_layout == 'masonry') echo '</li>'; ?>

			<?php endwhile; ?>

			<?php if ($blog_layout == 'masonry') echo '</ul>'; ?>

			<?php knacc_paging_nav(); ?>

		<?php else : ?>

			<?php get_template_part( 'content', 'none' ); ?>

		<?php endif; ?>

		</main><!-- #main -->
	</section><!-- #primary -->

<?php get_sidebar(); ?>
<?php get_footer(); ?>
